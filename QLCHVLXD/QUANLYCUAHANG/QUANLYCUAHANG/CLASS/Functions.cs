...
namespace QUANLYCUAHANG.CLASS
{
    public class Functions
    {
        ...
        public static void Connect()
        {
            ...
            Con.ConnectionString = @"Server = DELL-PC\SQLEXPRESS; Database = QLCHVLXD; Trusted_Connection = True;";
            Con.Open();                   //Mở kết nối
            //Kiểm tra kết nối
        ...
        public static string ChuyenSoSangChu(string sNumber)
        {
            ...
            mLengh = sNumber.Length - 1; // trừ 1 vì thứ tự đi từ 0
            for (int i = 0; i <= mLengh; i++)
            {
                mDigit = Convert.ToInt32(sNumber.Substring(i, 1));
                mTemp = mTemp + " " + mNumText[mDigit];
                if (mLengh == i) // Chữ số cuối cùng không cần xét tiếp 
                    break;
                switch ((mLengh - i) % 9)
                {
                    case 0:
